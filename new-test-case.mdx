...
## Have AI agent suggest and auto-generate more tests

Similar to the inital **AI discovery** we can run the discovery feature to "spawn" individual test cases.

Go into the test case and hover over the `suggest more` icon - a button will appear. Click on it.
...
AI agent will discover up to 3 new tests following up on the test case you launched it from. The original test will be added as a **dependency** automatically. It will auto-generate steps for every discovered test case and validate them.

AI agent status indicator will display what's going on.
...
The new test will appear grouped by the originating test. The AI agent has auto-generated test steps, validated them and where the validation succeeded, it turned the test **ON** - [into active mode](/execute-test-cases).

Help the AI agent when it quite nail the auto-generation - **yellow alert** hightlights a failed validation. See how in the [edit test case](/edit-test-case) section.
