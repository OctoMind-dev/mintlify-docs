---
title: How Octomind works
description: "Quick explainer on how to get stable, fast and affordable UI tests"
icon: "TV"
---

Watch a 3 min video demo:

<iframe
  width={560}
  height={315}
  src="https://www.youtube.com/embed/CGaJHpLAIq8"
  title="YouTube video player"
  frameBorder={0}
  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
  allowFullScreen
></iframe>

## 1. All we need is your website URL

... to start the AI powered test auto-discovery. We will discover, run and maintain your UI tests.
You can [try it right now](https://app.octomind.dev/setup/url?utm_source=docs&utm_medium=txt-lnk) by setting up your first user flow at **app.octomind.dev**.

<Frame caption="First page of the setup flow - link to your website, screenshot 09/2023">
  <img src="/images/setup/setup-1-url.png" alt="enter your url" />
</Frame>

## 2. We will discover your user flows

Full disclosure - we auto-discover a **sign-in** user flow and do **reachability** checks for all links on your page, but we'll add more manually.
Or you can try out our brand new [free prompting feature](/discovery) beta release!

<Frame caption="Test user credentials input to test the sign-in flow, screenshot 09/2023">
  <img
    src="/images/setup/setup-3-credentials.png"
    alt="enter a test user, screenshot 08/2023"
  />
</Frame>

<div class="mt-8" />

<Frame caption="Free promting window for test case discovery, screenshot 09/2023">
  <img
    src="images/free-prompting-UX.png"
    alt="free prompting window for test case discovery"
  />
</Frame>

## 3. We will generate, host and run your test code

We will generate [Playwright test code](https://playwright.dev/), host and run the tests for you.

## 4. We will provide test results

You will see the [test results](/test-results) in the app and in your PR comments if you integrated us into your CI pipeline.

<Frame caption="Example of Octomind test results in a Pull Request comment, screenshot 06/2023">
  <img
    src="images/Octomind-test-results-commit-comments-GitHub-example.png"
    alt="Example of Octomind test results in a Pull Request comment"
  />
</Frame>

## 5. You can review the broken user flow

You can go through the details, check the error log and open the [Playwright Trace Viewer](https://playwright.dev/docs/trace-viewer-intro) to see where the user flow broke.

<Frame caption="Example of Playwright Test Viewer, screenshot 08/2023">
  <img
    src="images/Trace-viewer-screeshot-082023.png"
    alt="Example of Playwright Test Viewer"
  />
</Frame>

## 6. And debug locally, if needed

Debug locally with Debugtopus. [Debugtopus is our open source tool](debugtopus) enabling you to run tests locally for easier debugging.

<Frame caption="Open source Debugtopus on GitHub, screenshot 08/2023">
  <img
    src="images/debugtopus-open-source.png"
    alt="Open source debugging tool Debugtopus"
  />
</Frame>

## 7. We'll maintain your tests automatically

Our AI will maintain your tests when they break.
