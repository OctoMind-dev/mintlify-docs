...
Our token handling mechanism has changed, so you need to adapt the YAML of the pipeline and replace the token in your secrets with a new token.
We are working on self-service; in the meantime please contact us for the new token.
