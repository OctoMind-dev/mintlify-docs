...
The following sections describe how to update between breaking-change versions of the pipeline.

### V1 → V2

Our token handling mechanism has changed, so you need to adapt the YAML of the pipeline and replace the token in your secrets with a [newly created API Key](#1-create-an-api-key).
You also need to add the `testTargetId`, which you can get as described [here](#2-copy-your-test-target-id).
