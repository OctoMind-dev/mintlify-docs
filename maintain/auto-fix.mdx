...
This is how to proceed when you [diagnosed a broken test](/guides/test-debugging-4.mdx).

1. Go to `test report` , into `test result` detail and click on the `auto-fix` tab.
2. Have **Agent Bubbles** attempt the auto-fix by clicking on the `attempt auto-fix` button.
...
- decline the auto-fix and [edit the test case manually](/maintain/maintain-tests#correct-the-broken-test-steps)
- try [editing the prompt](/guides/test-debugging-2)
