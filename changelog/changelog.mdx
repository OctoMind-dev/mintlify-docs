...
- `Agent upgrade`: The identifies a hover interaction and acts accordingly when discovering and generating test cases when you prompt it to
...
- We added Playwright `type characters` step to support rich text editors for fields where you type in manually.
- `global step timeout`: to shorten the timeout between steps if you have to run the tests faster
