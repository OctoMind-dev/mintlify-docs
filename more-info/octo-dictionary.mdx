...
A [project](/account/projects) is a collection of test cases and test reports tied to a certain app (the test target, URL).
...
[Recording](/build-tests/recorder) is another way to add a new test case to Octomind.

### Run locally

You can [run one or all test cases on your local machine](/run-tests/execution-local) against any test target.
...
Every test run will produce a [test report](/manage-tests/test-reports) where you will see all test results. They will tell you if everything runs as it's supposed to or if something is broken.
