...
A [project](/account-view/projects) is a collection of test cases and test reports tied to a certain app (the test target, URL).
...
[Recording](/get-started/new-test-case#record-a-test-case) is another way to add a new test case to Octomind. It is best used via **Playwright Codegen** and allows to paste code directly.

### Run locally

You can [run one or all test cases on your local machine](/get-started/debugtopus) against any test target.
...
Every test run will produce a [test report](/account-view/test-reports) where you will see all test results. They will tell you if everything runs as it's supposed to or if something is broken.
