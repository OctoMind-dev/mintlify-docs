...
We generate tests on [sign-up](/get-started/create-first-tests#4-open-the-octomind-app-for-the-first-time), when [we autogenerate 3 initial test cases](/get-started/create-first-tests#5-we-are-auto-generating-test-cases-for-you) and when you ask our AI agents to [generate more](/build-tests/autogenerate-tests#spawing-existing-test-cases) tests.
...
Our open source tool [Debugtopus](https://github.com/OctoMind-dev/debugtopus) can pull the latest test case from our repository and execute it against your local environment. [Learn how.](/run-tests/execution-local)
...
See [our section about prompting a new test case](/build-tests/prompt-agent#free-prompting-best-practices).
...
Lean more about our [CI integration options](/integrations/integrations-overview).
...
You can also run us [programmatically without using a CI](/run-tests/execution-curl), schedule regular test runs or trigger test runs manually.
...
## 12. Why does the Agent fail?

There are several reasons why the Agent might fail to generate a test case. [This article](/guides/agent-fails) describes the most common reasons in detail.

## 13. What types of interactions and assertions can the Agent handle?

- **interaction types:** enter text, press key (partially), click, select option, hover
- **assertion types:** visible, contains texts, have style
