...
Test script is just a part of the job setting up a working test suite needs a bit more. We've built Octomind to make your set-up fast and as little painful as possible.

## Reaching your app

1. **Allowing Octomind bot access your application**
...
3. **Configure exception for avoid rate limiting**
...
You can configure testing to match specific deployment settings and validate changes across different stages. You'll find `environments` settings it the left project menu.
...
Follow the respective step-by-steps to integrate Octomind tests to your CI pipeline.
...
- [cross origin iFrames](/advanced/cross-origin-iframes) - to temporary disable cross origin iframes
