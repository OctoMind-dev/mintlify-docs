...
You can decide what proxy servers to use for a specific test target and a specific environment. This is [how to access from different geographies or a private location](/proxy/proxy-settings).
...
After creating a project, Octomind provides you with a `default environment` with the url you provided during setup. You can create [multiple environment and set them up separately](/manage-tests/environments).
...
Variables are a helpful tool to handle test data and isolate. With Octomind, you can chose from a set of [pre-defined variables, create custom ones or dynamic variables](/advanced/variables). They can be set-up in the left project menu under the `environments` and `global variables` tab in each environment.

## Authentication

If your application requires a user to be logged in to access most of its functionality, you'd need your tests to login. Check out how to set up [basic authentication](/advanced/basic-authentication) or [OTP](/advanced/2fa) including [email flows](/advanced/email-flows).

To avoid load at test runtime we employ a shared auth state by sharing cookies between multiple test runs.

## CI integration

Octomind supports a variety of different [CI/CD providers](/integrations/) with native integrations and via [cURL command](/integrations/quickstart-Other.mdx).
...
- [global step timeout](/get-started/execute-tests#test-timeout) - if your test steps need more time to run
...
We run all [active tests in your test suite](/manage-tests/test-cases#activate-your-tests-for-execution) unless you use `tags` for [test sharding](/manage-tests/test-cases#use-tags) to chop your test execution into smaller chunks.

## Teardown

Teardown automatically cleans up artifacts created during test execution. This cleanup can happen through UI interactions (using the app's interface) or programmatically via javascript step.

Learn [how to onfigure test teardown here](/advanced/teardown).
