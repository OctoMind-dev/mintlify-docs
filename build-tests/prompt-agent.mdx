...
4. These are the limitations so far:
   ...
   - We can only handle flows which happen inside the app and in certain cases navigate to different urls. Cross-device flows are not supported. We are able to test [email confirmation flow](/advanced/email-flows.mdx) with an Octomind provided email address.
