...
This means any changes you make to the YAML files locally—whether through the CLI, your IDE, or any text editor—will be reflected in Octomind once they're merged and pushed.
...
jobs:
  sync-tests:
    ...
    steps:
      ...
      - name: Push tests to Octomind
        uses: OctoMind-dev/automagically-action-push@v1
        with:
...
1. **Copy from YAML view**: In the Octomind UI, view the test case in YAML format and copy it to your local repository
2. **Pull changes**: Run `octomind pull` to fetch all test cases from Octomind and update your local YAML files
