...
graph TD
    ...
    D -->|merge to main| E[GitHub Action executes tests]
    E -->|on success| F[GitHub Action pushes to Octomind]
    F -->|sync| G[Octomind Platform updated]
    G -->|changes in UI| H[Auto-commit to repo]
    H -->|sync| B
```
...
jobs:
  sync-tests:
    ...
    steps:
      ...
      - name: Push test results to Octomind
