...
While Octomind is supporting a login with a **one time password (OTP)**, one of the most likely limits that a test run will hit, is how often the same OTP code can be used for login.

## Rate limiter

If your application is protected against [DoS Attacks](https://en.wikipedia.org/wiki/Denial-of-service_attack) with a [rate limiter](https://en.wikipedia.org/wiki/Rate_limiting) it is very likely that Octomind test runs and/or discoveries will fail.
