...
If you also want to install auto completion (see below), that you should use a symlink in favor of an alias.

<Tip>
  Please make sure that there is no `package.json` in the `~/.local/packages` directory or above as this 
  will change the where npm will install the package.
</Tip>
