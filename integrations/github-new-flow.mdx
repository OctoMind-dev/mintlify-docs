...
<Info>
  We only request the permissions required to create and update pull request
  comments / checks. We do not need access to your code.
</Info>
...
<Warning>
  Make sure the `url` points to a publicly accessible deployment (e.g. preview
  deployment). Otherwise Octomind cannot run tests or exploration.
</Warning>
