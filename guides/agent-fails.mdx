...
The prompt must be sufficiently precise and encompass the steps needed to achieve the goal, similar to instructions given to a manual tester. Occasionally, a brief description or even a single word (e.g., "Log out") can suffice to compose comprehensive test cases, particularly if the application is designed intuitively. However, this is often not the case. If the prompt is too vague, deviates significantly, or requests interactions not present on the page, the agent will go wild. You will notice.
...
The agent relies on **screenshots and lists of extracted interactions** to determine the next best action. Unfortunately, current models cannot translate descriptions into x/y coordinates for mouse movements. To address this, we enhance our screenshots with colored boxes and IDs and supplement the list of interactive elements with as much contextual information as possible. This enriched screenshot and list should enable the agent to pinpoint the click target.

However, the absence of ARIA labels, labels, placeholder text, or test IDs on the page makes it extremely difficult, if not impossible, for the agent to identify the correct element. This often manifests as locators like "nth element of object unknown." Even copy-and-paste errors during app development, like naming a button "copy_of_settings" instead of "close_window," can cause problems.
...
If you report this problem to [Playwright maintainers](https://github.com/microsoft/playwright) directly, they will (rightfully) tell you that your app is not built to specifications.
...
When generating tests cases our AI agent to starts by navigating and crawling the application under test using the `Octomind bot`. Pages with strict bot protection can block it. [Here,](/advanced/octomind-bot) you'll find all necessary information you'll need to allow the Agent to interact with your application.
