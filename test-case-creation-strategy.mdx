...
You can have the AI agent auto-discover more test cases. Go into the test case and hover over the `suggest more` icon - a button will appear. Click on it. Find more on how `suggest more` works [here](/new-test-case).

The AI agent will discover up to 3 new tests based on the one you started it from. The original test will be added as **dependency**. We will auto-generate steps for every discovered test case and validate them.
...
At sign-up, we automatically discover **close cookie banner** and **"required" login** test cases. These are often the necessary prerequisites for all other tests. [Learn more.](/first-steps#4-open-the-octomind-app-for-the-first-time)
