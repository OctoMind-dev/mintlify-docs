...
2. Always use "Discover test cases" from the overview page. It gives you a headstart.
...
3. Use "create custom test cases" from the overview page.
   <Frame caption="Start test case creation formt he overview page, screenshot 02/2024">
     <img
...
The sign-up procedure also creates a test cases to close the cookie banner, if applicable while the login test case typcially opens the door for the high value user flows.

<Tip>

**Advantages of starting with a prompt**

- All these techniques will give you a couple of test cases to get startet with
...
Keep test cases short and to the point. Ideally each test case represents a small tasks you want to achieve. One technique to make this happen are dependencies.
Typically the first test case you need is "accept all cookies". Second one is "login with username and password", third one might be "open profile page".
The second test case would have to take care of cookies. The third test case would have to take care of cookies and login.
This is because every test case runs in isolation and therefore always needs to be the full flow.
This is where dependencies - or test case chaining - gets really valuable.
Make "accept all cookies" a dependency of "login with username and password" and make "login with username and password" a dependency of "open profile page".
...
1. The `accept cookies` tes case does not have dependencies since it is the first test case of them all.
...
2. The `login with username and password` test case is based on the `accept cookies` test case.

   <Frame caption="login with username and password is based on the accept cookies, screenshot 02/2024">
     <img
       src="images/dependency-02-login.png"
       alt="login with username and password is based on the accept cookies"
       width="300px"
     />
   </Frame>

3. The `open profile` test case is based on the `login with username and password` test case.
   <Frame caption="Open profile is based on login with username and password, screenshot 02/2024">
     <img
       src="images/dependency-03-open-profile.png"
       alt="Open profile is based on login with username and password"
       width="300px"
     />
   </Frame>
...
We are constantly improving the agent but it still might go wrong. In this case you need to help it find its way.
Use the step inspector to fine tune interactions and assertions.

### Add missing steps

You can add missing steps by clicking the `+` icon and choosing the desired intearction or assertion.
...
<Frame caption="delete obsolet steps, screenshot 02/2024">
  <img
    src="images/steps-delete-step.png"
    alt="delete obsolet steps"
    width="300px"
  />
</Frame>
...
<Tip>
  **Advantages of helping out the agent**
  ...
  - modifying steps manually is tpically much faster then changing the prompts
...
You can find the `run locally` in the debug section of your test case.
...
You can use the locator tool to easily receive CSS locators. Activate it and click on the desired element.
Then you can copy the CSS locator.

<Frame caption="run locally: CSS locator tool 02/2024">
  <img
    src="images/run-locally-Playwright-selector.png"
    alt="run locally: CSS locator tool"
  />
</Frame>
...
Run locall will speed-up your test creation efforts noticable.

</Tip>

## Outlook

Expect the process to create test cases to change substantially over the next weeks and months. We are working
